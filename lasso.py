...
path = f"predicted_dataset/lasso/GW{gameweek}"
...
if largest_gw > gameweek:
    test_inputs = torch.tensor(test.values, dtype=torch.float32)  
...
if 'lasso' in predictions_df2.columns:
    if pd.isnull(predictions_df2.loc[gameweek, 'lasso']) or predictions_df2.loc[gameweek, 'lstm']==0:
